...
function App() {
  ...
  return <div className="site-shell" id="top">
    ...
    {!page ? <main className="home"><p className="eyebrow">HAOBI · AI OPERATIONS FOR REAL WORK</p><h1>Ba AI.<br /><em>2 BI.</em></h1><p className="lead">A → B: từ AI đến BI, từ dữ liệu đến quyết định. HaoBi biến quy trình doanh nghiệp thành một đội ngũ AI làm việc thật.</p><a className="button" href="#setup">Khám phá 6 hướng <span>↗</span></a><div className="page-grid">{pages.map((item) => <a className="page-card" href={`#${item.slug}`} key={item.slug}><span>{item.number}</span><strong>{item.title}</strong><small>{item.offer}</small></a>)}</div></main> : <main className="detail"><p className="eyebrow">{page.number} / 06 · {page.kicker}</p><h1>{page.title}</h1><p className="lead">{page.description}</p><div className="detail-grid"><div><p className="eyebrow">HAOBI LÀM GÌ</p><ul>{page.points.map((point) => <li key={point}>{point}</li>)}</ul></div><aside><span className="mono">MÔ HÌNH</span><strong>{page.offer}</strong><a className="button" href="mailto:lienhe@haobi.io.vn?subject=Trao đổi HaoBi">Trao đổi cùng HaoBi <span>↗</span></a></aside></div><nav className="pager" aria-label="Các hướng HaoBi">{pages.map((item) => <a className={item.slug === page.slug ? 'active' : ''} href={`#${item.slug}`} key={item.slug}>{item.number}</a>)}</nav></main>}
