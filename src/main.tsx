...
const commands = ['scan inbox', 'read dashboard', 'draft report', 'wait approval'];
...
function App() {
  ...
  return <div className="site"><div className="particles" aria-hidden="true">{particles.map(i=><span key={i} style={{'--i': i} as React.CSSProperties}/>)}</div>
    ...
    <main id="top">
      <section className="hero section"><motion.div className="scrollbar" style={{ scaleX: 1 }} />
        <div>
          ...
          <Reveal delay={.05}><h1>Một hệ thống AI riêng cho công việc thật.</h1><div className="verb-pill">AI đang <motion.span key={verb} initial={{y:18,opacity:0,filter:'blur(8px)'}} animate={{y:0,opacity:1,filter:'blur(0)'}}>{verbs[verb]}</motion.span></div></Reveal>
          <Reveal delay={.1}><p className="lead">HaoBi giúp cá nhân và doanh nghiệp nhỏ triển khai OpenClaw, Power BI và automation thành một đội AI nhỏ: biết đọc dữ liệu, tạo báo cáo, theo dõi việc lặp lại và chờ bạn phê duyệt trước khi hành động.</p></Reveal><Reveal delay={.14}><div className="stats"><strong><Count to={1} /></strong><span>workspace riêng</span><strong><Count to={5} /></strong><span>lớp workflow</span><strong><Count to={100} suffix="%" /></strong><span>human approval</span></div><div className="command-loop">{commands.map((x,i)=><motion.code key={x} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*.12}}>hao.bi / {x}</motion.code>)}</div></Reveal>
        ...
        <Reveal delay={.12}><div className="map" aria-label="Workflow map"><motion.div className="pulse" animate={reduce ? {} : { scale: [1, 1.08, 1], opacity: [.45, .75, .45] }} transition={{ duration: 3.2, repeat: Infinity }} /><div className="dashboard-mini"><b>Today</b><span>3 drafts</span><span>2 alerts</span></div><div className="you">Founder OS</div>{['Data','Agent','Report','Approval','Automation'].map((x,i)=><motion.div key={x} className={`node n${i}`} animate={reduce ? {} : { y: [0, -5, 0] }} transition={{ duration: 3 + i*.35, repeat: Infinity, delay: i*.15 }} whileHover={reduce ? {} : { scale: 1.08, rotate: i % 2 ? 2 : -2 }}>{x}</motion.div>)}<div className="mini-card m1"><b>Power BI</b><span>Revenue variance +12%</span></div><div className="mini-card m2"><b>Approval</b><span>Send follow-up?</span><em>Review</em></div><div className="mini-card m3"><b>n8n</b><span>Daily brief ready</span></div><svg viewBox="0 0 420 420"><path d="M210 210 L90 110 M210 210 L330 110 M210 210 L90 315 M210 210 L330 315 M210 210 L210 60"/><circle cx="210" cy="210" r="118"/><circle cx="210" cy="210" r="165"/></svg></div></Reveal>
      </section>
